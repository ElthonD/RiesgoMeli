...
def createPage():
    ...
    def load_HR():
        ...
        rutaA = r'./data/Historico de Robos Mercadolibre.xlsx'
    ...
    try:
        ...
        st.markdown("<h4 style='text-align: left;'>Paso 3: Consultar Histórico de Robos Mercadolibre (Planner) </h4>", unsafe_allow_html=True)
        st.write(""" 
        La finalidad de este módulo es consultar el histórico de robos de Mondelez. Pasos a seguir para este módulo:
        1. Seleccionar el **Mes** del cuál desea obtener información en el mapa. Seleccionando el checkbox, puede seleccionar todos los meses del año que presenten robos de Mercadolibre.
        2. Seleccionar el **Dia** del cuál desea obtener información en el mapa. Seleccionando el checkbox, puede seleccionar todos los días del año que presenten robos de Mercadolibre.
        3. El resultado indica:
        + Icono con el Logo de Mercadolibre donde se presentan todos los robos y ubicados por Estados de la República Mexicana.
        + Información sobre el robo (Origen, Destinos, Estado, Tipo evento (Recuperado/Consumado), Mes, Día, Hora).
        """)

        df3 = load_HR()
        st.write(f"Este módulo contiene información histórica de los robos de Mercadolibre desde **{df3.Mes.values[0]} {df3.Año.values[0].astype(int)}** a **{df3.Mes.values[-1]} {df3.Año.values[-1].astype(int)}** :")
        ...
        st.markdown("<h5 style='text-align: left;'>Mapa de Robos Mercadolibre</h5>", unsafe_allow_html=True)

        mapa_coropleta = map_coropleta_fol(df_selected_dia)

        st.markdown("<h4 style='text-align: left;'>Paso 4: Zonas de Riesgo Mercadolibre</h4>", unsafe_allow_html=True)
        st.write(""" 
        La finalidad de este módulo es visualizar la frecuencia de robos por días y horas por las zonas de riesgo en el comportamiento histórico por meses de los robos de Mercadolibre. 
        """)
